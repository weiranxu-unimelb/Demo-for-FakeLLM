...
### 2. 推荐方案 A：局域网内直接访问
在同一 Wi-Fi 或公司局域网下，你可以直接使用手机浏览器访问该部署机所在局域网 IP（比如 `http://192.168.1.xxx:5173`）进行访问。
前提是你确保外面对该端口不予放行，保障系统处于可信局域网环境内。

### 3. 推荐方案 B：Ngrok 免费 HTTPS 公网代理穿透 (移动端外出可用)

如果在无公网 IP、非局域网环境下想用手机随时随地访问，我们推荐使用**基于 Ngrok 免费 HTTPS 域名 + WebSocket 的自建内网代理方案**。

本项目的 `docker-compose.yml` 中已捆绑了包含完整配置的 `ngrok` 与 `shadowsocks (v2ray-plugin)`。

**操作步骤：**
1. 注册获取你自己的 **Ngrok AuthToken** 及一个 **免费的 HTTPS 静态域名**。
2. 在项目根目录的 `.env` 中填入你的 Token 和 免费域名。
3. `docker-compose up -d` 启动后。
4. 在手机中下载如 **v2rayNG** / **Shadowrocket** 等应用，连接你自己的免费域名，并在插件配置处启用 WebSocket (`v2ray-plugin`)。
5. 连接代理成功后，你在公网即可随时随地使用手机浏览器输入 `192.168.x.x:5173` (你部署机的局域网 IP) 直接访问本项目。
